...
public class MetadataHolderCassandraItemProcessor implements ItemProcessor<String, CreateTableSpecification> {
    ...
    public CreateTableSpecification process(String item) throws Exception {
        ...
        final ColumnMetadata primaryKeyColumn = getPrimaryColumn(columnMetadataList);
        if (primaryKeyColumn == null) {
            throw new NoPrimaryKeyException(tableName);
        }
        final DataType primaryKeyDataType = getCassandraDataType(primaryKeyColumn.getDataType());
        CreateTableSpecification spec = CreateTableSpecification.createTable()
                .ifNotExists()
                .name(tableName)
                .partitionKeyColumn(primaryKeyColumn.getColumnName(), primaryKeyDataType)
    ...
    private ColumnMetadata getPrimaryColumn(List<ColumnMetadata> columnMetadatas) {
