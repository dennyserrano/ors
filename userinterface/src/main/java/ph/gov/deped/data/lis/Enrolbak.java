 ...
 */
@Entity
@Table(catalog = "lis2db", schema = "", uniqueConstraints = {
    @UniqueConstraint(columnNames = {"school_term_id", "school_id", "learner_registry_id", "duplicate_arbiter"})})
@XmlRootElement
@NamedQueries({
    @NamedQuery(name = "Enrolbak.findAll", query = "SELECT e FROM Enrolbak e")})
public class Enrolbak implements Serializable {
    ...
    public Enrolbak(Integer id) {
        this.id = id;
    }

    public Enrolbak(Integer id, short sy, long lrn, short enrolType, int schoolClassId, Date firstLoginDt, short repeater, short grade1Ecd, short balikAral, short muslim, short aliveEnrolled, short giftedTalented, int schoolTermId, int schoolId, Date updatedAt) {
        this.id = id;
        this.sy = sy;
        this.lrn = lrn;
    ...
    public boolean equals(Object object) {
        ...
        if (!(object instanceof Enrolbak)) {
            return false;
        }
        Enrolbak other = (Enrolbak) object;
    ...
    public String toString() {
        return "ph.gov.deped.data.lis.Enrolbak[ id=" + id + " ]";
    }
